...
//Bir Karakter Dizisini Baska Bir Karakter Dizisine Kopyalama__strcpy
/*
...
//Karakter Dizilerini Karsilastirmak___strcmp
...
/*    char metin1[20],metin2[20];
    ...
    if(strcmp(metin1,metin2))
        printf("metinler birbiriden farkli");

    else
        printf("metinler birbiri ile ayni");
    printf("\n");

*/


/*    char* isimler1[] = {"ali", "ayse", "mehmet", "fatma"};
    char* isimler2[] = {"ali", "ece", "erkan", "fatma"};
    int boyut1 = sizeof(isimler1) / sizeof(isimler1[0]);//dizinin toplam boyutu dizinin bir elemaninin boyutuna bolunur!!!
    int boyut2 = sizeof(isimler2) / sizeof(isimler2[0]);

    if (boyut1 != boyut2) {
        printf("Dizi boyutlari esit degil!\n");
        return 1;
    }

    for (int i = 0; i < boyut1; i++) {
        if (strcmp(isimler1[i], isimler2[i]) != 0) {
            printf("%s ve %s metinler birbirinden farkli\n", isimler1[i], isimler2[i]);
        } else {
            printf("%s metinleri ayni\n", isimler1[i]);
        }
    }
*/
...
/*
    char isimler1[4][10] = {"ali", "ayse", "mehmet", "fatma"};
    char isimler2[4][10] = {"ali", "ece", "erkan", "fatma"};
    int boyut1 = sizeof(isimler1) / sizeof(isimler1[0]);
    int boyut2 = sizeof(isimler2) / sizeof(isimler2[0]);
    ...
    for (int i = 0; i < boyut1; i++) {
        if (strcmp(isimler1[i], isimler2[i]) != 0) {
            ...
            printf("%s ve %s metinleri ayni\n", isimler1[i], isimler2[i]);
        }
    }
*/


//Slayt_ORNEK_11______GirilenCumleyiTersYazdirma
...
/*    char cumle[50];
    ...
    for(i=strlen(cumle);i>=0;i--)
        {//strlen terminating karakteri de sayar\0
        printf("%c",cumle[i]);
    }
*/
...
/*    char cumle_dizi [50],aranacakHarf;
    ...
    int sayac = 0; // Aranan harfin kaç kez bulunduğunu tutacak değişken

    for (int i = 0; cumle_dizi[i] != '\0'; i++) {
        if (cumle_dizi[i] == aranacakHarf) {
            sayac++;
        }
    }
    printf("Cumlede %d adet %c harfi bulunur",sayac,aranacakHarf);
*/
    ...
    // Kelimeyi hecelere ayırma
    printf("Hecelere ayrılmış hali: ");
    for (int i = 0; i < uzunluk; i++) {
        ...
        for (int j = 0; j < strlen(sesli_harfler); j++) {
            if (kelime[i] == sesli_harfler[j]) {
                if (i != uzunluk - 1) {
                    printf("-"); // Sesli harften sonra tire ekleyerek heceyi ayırıyoruz
                }
                break;
            }
        }
